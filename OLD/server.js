...
function classifyPlace(place, keywords) {
  ...
  const category = (place.category || '').toLowerCase(); // 카카오 카테고리 추가
  const combined = (nameL + ' ' + addrL + ' ' + category).toLowerCase();

  // 우선순위: 카테고리 체크 (카카오맵 공식 카테고리)
  if (category.includes('반려견놀이터') || category.includes('dog run') || category.includes('도그런')) {
    return '도그런';
  }
  if (category.includes('수영장') || category.includes('계곡') || category.includes('물놀이')) {
    return '수영장';
  }
  if (category.includes('펫카페') || category.includes('애견카페')) {
    return '카페';
  }
  if (category.includes('펜션') || category.includes('글램핑')) {
    return '숙박';
  }
  if (category.includes('미용') || category.includes('그루밍')) {
    return '미용';
  }
  if (category.includes('동물병원') || category.includes('수의')) {
    return '병원';
  }
  if (category.includes('용품') || category.includes('펫샵')) {
    return '펫샵';
  }
  if (category.includes('공원') || category.includes('근린공원')) {
    return '공원';
  }

  // 우선순위 2: 이름/주소 패턴 매칭 (name + address)
  const patterns = [
    { cat: '도그런', re: /도그런|dog\s*run|언더독|반려견\s*체험|펫\s*플레이|강아지\s*운동장|애견\s*운동장|반려견놀이터/ },
    { cat: '수영장', re: /수영장|물놀이|워터파크|펫\s*풀|계곡|물\s*근처|래프팅/ },
    { cat: '카페', re: /펫카페|애견카페|반려견\s*카페|도그\s*카페|카페|café|coffee/ },
    { cat: '식당', re: /레스토랑|음식점|식당|펍|바|식음|구이|국밥|면|밥|음식/ },
    ...
    { cat: '펫샵', re: /펫\s*샵|애견\s*용품|강아지\s*용품|견생냥|유폴|아무도없개/ },
  ];
...
function generateCoursesByRules(places, startLat, startLng, activity, duration, durConfig, dogName, dogSize) {
  ...
  const validPlaces = places.filter(p => {
    const isDist = p.distance >= durConfig.minKm && p.distance <= durConfig.maxKm;
    const hasCoords = !isNaN(p.lat) && !isNaN(p.lng);
    return isDist && hasCoords;
  });

  console.log(`거리 필터링: ${places.length}개 → ${validPlaces.length}개`);

  if (validPlaces.length === 0) {
    console.warn(`⚠️ 유효한 장소 부족 (거리 범위: ${durConfig.minKm}~${durConfig.maxKm}km)`);
    return [];
  }
  ...
  validPlaces.forEach(p => {
    try {
      ...
      console.error(`카테고리 분류 오류 (${p.name}):`, e.message);
      if (!categorized['기타']) categorized['기타'] = [];
      categorized['기타'].push(p);
    }
  });

  const catSummary = Object.keys(categorized).map(k => `${k}:${categorized[k].length}`).join(', ');
  console.log(`카테고리 분류 완료: ${catSummary}`);

  let courseTemplates = [];
  try {
    courseTemplates = getCourseCombinations(activity, categorized);
    console.log(`코스 템플릿 생성: ${courseTemplates.length}개`);
  } catch (e) {
    console.error('코스 템플릿 생성 오류:', e.message);
    return [];
  }
  
  if (courseTemplates.length === 0) {
    console.warn('⚠️ 코스 템플릿 생성 불가');
    return [];
  }
  ...
  for (let i = 0; i < courseTemplates.length && courses.length < 5; i++) {
    ...
    try {
      ...
      if (course) {
        ...
        console.log(`✅ 코스 ${courses.length} 생성 완료: ${course.title}`);
      }
    } catch (e) {
      console.error(`코스 빌드 오류 (템플릿 ${i}):`, e.message);
    }
  }

  console.log(`🎯 최종 코스 생성 완료: ${courses.length}개`);
  return courses;
}
...
function getCourseCombinations(activity, categorized) {
  ...
  if (activity === '냄새 탐험') {
    ...
    } else if (parks.length > 0 && restaurants.length > 0) {
      // 카페 없으면 식당으로 폴백
      for (let i = 0; i < Math.min(3, parks.length); i++) {
        for (let j = 0; j < Math.min(3, restaurants.length); j++) {
          templates.push({
            primary: { cat: '공원', idx: i },
            secondary: { cat: '식당', idx: j },
            title: '산책 후 식사'
          });
        }
      }
    ...
    } else if (sports.length > 0 && restaurants.length > 0) {
      // 카페 없으면 식당으로 폴백
      for (let i = 0; i < Math.min(3, sports.length); i++) {
        for (let j = 0; j < Math.min(3, restaurants.length); j++) {
          templates.push({
...
app.post('/api/generate-course', async (req, res) => {
  ...
  try {
    async function searchKakaoMultiRadius(keywords, centerLat, centerLng, maxRadius) {
      const results = [];
    ...
    const [kakaoResults, tourResults] = await Promise.all([
      searchKakaoMultiRadius(minKm > 20 ? (config.keywordsLong || config.keywords) : config.keywords, lat, lng, radius),
    ...
    if (unique.length === 0) {
      return res.status(404).json({ error: `${durConfig.label} 범위에 ${activity} 장소가 없어요. 다른 활동이나 거리를 선택해보세요.` });
    }
...
app.get('/health', (req, res) => {
  res.json({ status: 'ok', message: '댕댕로드 서버 정상 작동 중 🐾', algorithm: 'rule-based', caching: false });
});
...
app.listen(PORT, () => {
  console.log(`🐾 댕댕로드 서버 시작! (규칙기반 알고리즘, 캐시 없음) http://localhost:${PORT}`);
});
